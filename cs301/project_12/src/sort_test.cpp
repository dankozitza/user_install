...
int main(int argc, char* argv[]) {
   Value *fuzz1, *fuzz2; // Arrays of random values to be sorted.
   ...
   // set fuzz2 values for insert sort
   for (int i = 0; i < f_size; ++i)
      fuzz2[i] = fuzz1[i];

   count = sorters::insert<Value, size_t, unsigned long long>(fuzz2, f_size);
   ...
   // set fuzz2 values for merge sort
   for (int i = 0; i < f_size; ++i)
      fuzz2[i] = fuzz1[i];
   ...
   // set fuzz2 values for quick sort
   for (int i = 0; i < f_size; ++i)
      fuzz2[i] = fuzz1[i];

   count = sorters::quick<Value, size_t, unsigned long long>(fuzz2, f_size);
